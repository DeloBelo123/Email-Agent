...
import { sendSession } from "../../../myLibUI/Backend/supabase/supabase"
import { PythonResponse, NextJSResponse, Email } from "./BackendResponseTypes"
...
export function Mails({category}:{category: MailCategories}){
    //sending the session to my KI-Agent
    const { mutate, error:postError } = useMutation({
        mutationFn: async () => {
            return await sendSession<PythonResponse>({
    ...
    const { data:mails, isLoading, error:getError } = useQuery({
        queryKey:["mails",category],
        queryFn:async()=>{
            const data = await axios.get<NextJSResponse>(`/api?category=${category}`)
            return data.data.data
        }
    })
    if(getError) throw new Error("ein fehler beim next.js backend fetchen: " + getError) 
    if(isLoading) return <p>Loading...</p>
    return(
        <div className=" flex flex-col gap-2 border">
            {mails?.map((mail:Email)=>{
                return(
                    <div className=" flex flex-col gap-2" key={mail.unique_mail_id}>
                        <div className=" flex flex-col gap-1">
                            <p>von:{mail.mail_header}</p> 
                        </div>
                        <div>
                            <p>zusammenfassung:{mail.mail_summary}</p>
                            <p>ganze Email:'kommt noch'</p>
                        </div>
                        <div className=" flex flex-col gap-1">
                            <p>email_id:{mail.unique_mail_id}</p>
                            <p>email_besitzer_id:{mail.inhaber_id}</p>
                        </div>
                    </div>
                )
            })}
